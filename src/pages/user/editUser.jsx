...
class EditUser extends React.Component {
  ...
  editSubmit = () => {
    this.props.form.validateFields((err, fields) => {
      ...
      if (err) {
        ...
        let reqParam = {...fields};
        reqParam.id = this.props.record.id;
        let isAfterDelete = this.state.isAfterDelete
        if (isAfterDelete) {
          reqParam.headImage = this.state.headImage
        } else {
          reqParam.headImage = this.state.headImage || this.record.headImage;
        }
  ...
  delImg = () => { // 删除上传的图片
    if (this.record.headImage && !this.state.isAfterDelete) {
      this.setState({
        isAfterDelete: true,
        ifShowDelBtnBj: {
          'opacity': 0,
  ...
  render() {
    ...
    let record = this.record = this.props.record;

    let isUpload = this.isUpload  = !!(this.state.isAfterDelete ? this.state.headImage : (this.state.headImage || record.headImage));
          ...
          >
            {
              <div onMouseEnter={this.handleMouseEnter} onMouseLeave={this.handleMouseLeave}>
                <img src={this.state.isAfterDelete ? this.state.headImage || defaultHeadImage : (this.state.headImage || record.headImage|| defaultHeadImage)} alt="头像"/>
                <div
