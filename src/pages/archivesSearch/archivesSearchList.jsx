...
export default class BeforeBooklet extends React.Component {
        ...
        return (
          <span>
            ...
            </Tooltip>
            {
              ...
              &&
              <Tooltip title="收藏">
                <Icon type="heart-o" style={{color: 'red'}} className="operate-icon"
                      onClick={() => this.favouriteAdd(record)}/>&nbsp;
              </Tooltip>
            }
            {
              record.collected === true
              &&
              <Tooltip title="取消收藏">
                <Icon type="heart" style={{color: 'red'}} className="operate-icon"
                      onClick={() => this.favouriteDel(record)}/>&nbsp;
              </Tooltip>
            }
          </span>
        )
  ...
  favoriteChange = (id, isCollect) => {
    let list = cKit.copyJson(this.state.pageList);
    let newList = this.recordListUpdate(list, id, isCollect);
    this.setState({
      pageList: newList
    });
  }
  ...
  getRecordsFetch = ({
    ...
    let successHandler = (response) => {
      if (response.code == cKit.ResponseCode.SUCCESS_CODE) {
        let {total, pageList, hasToForeignPermission} = response.datas;
        this.setState({
          page,
          size,
          keyword,
      ...
      <div>
        ...
        <div className="main-content main-content-animate">
          ...
          <div className="booklet-table-box">
            ...
            <CustomTable
              ...
              rightBottom={
                <div>
                  {
                    this.state.hasToForeignPermission === true
                    &&
                    pKit.hasPermission('REC_ARCHIVESEARCH_PROVIDEOUTER')
                    &&
                    <Button
                      disabled={this.state.selectedRowKeys.length <= 0}
                      onClick={this.onExternalOffer}
                      icon="export"
                      type="primary"
                    >对外提供
                    </Button>
          ...
          <ExternalOffer
            ...
            externalOfferRecords={this.state.selectedRowKeys}
          />
        </div>
      </div>
    )
