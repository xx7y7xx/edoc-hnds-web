...
export default class ArchivesSearch extends React.Component {
  ...
  getYearList = (orgId) => {
    let thiz = this;
    let url = '/recquery/year/dropdown?orgId=' + orgId;
    let successHandler = (response) => {
      if (response.code == cKit.ResponseCode.SUCCESS_CODE) {
        let yearList = response.datas;
        if(yearList.length > 0){
          let defaultYear = yearList[0].value;
  ...
  onLoadData = (treeNode) => {  // 左侧树 懒加载
    ...
    let reqParam = {accountYear: accountYear, accountMonth: accountMonth, current_org_id: this.state.orgId}
