...
class AddAccessApply extends React.Component {
  ...
  rejectSubmit = () => {
    this.props.form.validateFields((err, fields) => {
      ...
      if (err) {
        ...
        let approveNote = fields.approveNote;
        this.editRecordFetch({
          approveNote,
          success: (datas) => {
            let fn = this.props.success;
  ...
  editRecordFetch = ({isApprove, approveNote, success}) => {
    let successHandler = (response) => {
      if(response.code == cKit.ResponseCode.SUCCESS_CODE){
        let {msg, datas} = response;
    ...
    let url = isApprove? '/consult/approve' : '/consult/reject';
    let id = this.state.targetRecord.id;
    let json = {id};
    isApprove || (json.approveNote = approveNote);
    netKit.postFetch({
      url,
      data: [json],
      success: successHandler,
    ...
    return (
      <CustomModal
        ...
        operate={
          !this.props.isView && (
            ...
            <div>
              <Button type="primary" onClick={this.setApproveNote}>
              驳回
              </Button>
      ...
      >
        <div className="access-apply-form" ref="accessApplyForm">
          {this.state.currentAction == 'reject' ?
            <Form>
              <ValidatePopover validatePoppoverId="approveNote"
                  ...
                  <FormItem>
                    {getFieldDecorator('consultDate', {
                      ...
                      <CustomRangePicker
                        allowClear={false}
                        onChange={(value) => {
                          value.length && value[0].isAfter(value[1]) && value.reverse();
                          setFieldsValue({
                            consultDate: value
                          });
                        }}
